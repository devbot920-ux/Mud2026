...
import {ATTRIBUTE_NAMES,CLASSES,EQUIPMENT,RACES,attackDamage,characterAttributes,characterSkills,characterStats,effectiveArmor,findEquipment,gainExperience,inventoryWeight,receivedDamage,rewardForMob,updateEndurance,type EquipmentId,type EquipmentSlot,type PlayerClass,type PlayerRace} from "./rpg";
...
let openPanel:"inventory"|"character"|undefined,inputMode:"mouse"|"command"="mouse";
...
function renderGamePanel(){if(openPanel==="inventory")renderInventoryPanel();else if(openPanel==="character")renderCharacterPanel();}
function openGameMenu(panel:"inventory"|"character"){openPanel=panel;gamePanel.hidden=false;document.body.classList.add("menu-open");keys.clear();if(document.pointerLockElement)void document.exitPointerLock();renderGamePanel();}
...
function respawnPlayer(){const stats=currentCharacterStats();playerHealth=stats.maxHealth;playerEndurance=stats.maxEndurance;playerMana=stats.maxMana;combatTargetId=undefined;arenaOpponentId=undefined;combatTargetHealth=0;combatClock=enemyCombatClock=0;currentRoomId=4169;buildRoom();playerPosition.set(4.5,0,4.5);yaw=-Math.PI*3/4;showInteraction("You awaken in Pendelhaven Hospice","Vivian and the healers restore your health, movement, and mana. Your experience and equipment remain with you.");renderPlayerStatus();}
...
function advanceCombat(dt:number){const playerStats=currentCharacterStats(),attackInterval=playerStats.attackInterval;if(combatTargetId===3998){combatClock+=dt;if(combatClock<attackInterval)return;combatClock-=attackInterval;const damage=attackDamage(playerStats,equippedWeapon?EQUIPMENT[equippedWeapon]:undefined);dummyHealth=Math.max(0,dummyHealth-damage);spawnCombatText(`-${damage}`,"#ffd66b",dummyVisual?.position??new THREE.Vector3());showInteraction("You strike the wooden dummy",`Your attack deals ${damage} damage.\n\nDummy: ${dummyHealth}/30 HP`);if(dummyHealth===0){combatTargetId=undefined;interactionText.textContent+="\n\nThe battered dummy yields. Practice complete.";}renderPlayerStatus();return;}const stats=combatTargetId?hostileStats(combatTargetId):undefined;if(!stats)return;const targetId=combatTargetId!;combatClock+=dt;enemyCombatClock+=dt;let message="";if(combatClock>=attackInterval){combatClock-=attackInterval;const damage=attackDamage(playerStats,equippedWeapon?EQUIPMENT[equippedWeapon]:undefined);combatTargetHealth=Math.max(0,combatTargetHealth-damage);const targetPosition=roomInteractions.find(target=>target.id===targetId)?.position??new THREE.Vector3();spawnCombatText(`-${damage}`,"#ffd66b",targetPosition);message=`You strike the ${stats.name} for ${damage} damage.`;if(combatTargetHealth===0){completeHostileVictory(targetId,stats.name);return;}}if(enemyCombatClock>=stats.attackInterval){enemyCombatClock-=stats.attackInterval;const damage=receivedDamage(stats.damage,playerStats,equippedArmor?EQUIPMENT[equippedArmor]:undefined);playerHealth=Math.max(0,playerHealth-damage);spawnCombatText(`-${damage}`,"#ff5b4d",playerPosition);flashPlayerDamage();message+=`${message?"\n":""}The ${stats.name} hits you for ${damage} damage.`;if(playerHealth===0){respawnPlayer();return;}}if(message)showInteraction(`Fighting ${stats.name}`,`${message}\n\nYou: ${playerHealth}/${playerStats.maxHealth} HP · ${stats.name}: ${combatTargetHealth}/${stats.maxHealth} HP`);renderPlayerStatus();}
...
function go(edge: Edge) { if(currentRoomId===INITIAL_ROOM_ID&&edge.direction==="W")tutorialProgress.add("west"); currentRoomId=edge.to_room; buildRoom(edge); }
...
document.querySelectorAll<HTMLButtonElement>("[data-open-panel]").forEach(button=>button.addEventListener("click",()=>openGameMenu(button.dataset.openPanel as "inventory"|"character")));
...
addEventListener("keydown",e=>{if(["INPUT","SELECT","BUTTON"].includes((e.target as HTMLElement).tagName))return;if(e.code==="Enter"&&!e.repeat){e.preventDefault();setInputMode("command");return;}if(e.code==="Escape"&&openPanel){closeGameMenu();return;}if(e.code==="KeyI"&&!e.repeat){openGameMenu("inventory");return;}if(e.code==="KeyC"&&!e.repeat){openGameMenu("character");return;}if(openPanel)return;keys.add(e.code);if(e.code==="KeyE"&&!e.repeat)interactNearby();if(e.code==="KeyV"&&!e.repeat){vHeld=true;vZoomed=false;}if(e.key.toLowerCase()==="h"){showHidden=!showHidden;buildRoom(undefined,true);}if(e.key==="`"){developerVisible=!developerVisible;renderUi();}});addEventListener("keyup",e=>{keys.delete(e.code);if(e.code==="KeyV"&&vHeld){if(!vZoomed){thirdPerson=!thirdPerson;if(playerAvatar)playerAvatar.root.visible=thirdPerson;}vHeld=false;}});
