...
addEventListener("keydown",e=>{if(["INPUT","SELECT","BUTTON"].includes((e.target as HTMLElement).tagName))return;keys.add(e.code);if(e.code==="KeyE"&&!e.repeat)interactNearby();if(e.code==="KeyV"&&!e.repeat){vHeld=true;vZoomed=false;}if(e.key.toLowerCase()==="h"){showHidden=!showHidden;buildRoom(undefined,true);}if(e.key==="`"){developerVisible=!developerVisible;renderUi();}});addEventListener("keyup",e=>{keys.delete(e.code);if(e.code==="KeyV"&&vHeld){if(!vZoomed){thirdPerson=!thirdPerson;if(playerAvatar)playerAvatar.root.visible=thirdPerson;}vHeld=false;}});
