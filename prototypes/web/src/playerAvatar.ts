...
export function createPlayerAvatar(race:PlayerRace,playerClass:PlayerClass):PlayerAvatar{
  ...
  const raceScale=race==="Dwarf"?.82:race==="Kobold"?.74:1;const width=race==="Dwarf"?1.22:race==="Kobold"?.82:1;
  const classColor:Record<PlayerClass,number>={Warrior:0x53616b,Rogue:0x244c35,Mage:0x4b3571,Cleric:0xd5ccb4};
  const skinColor=race==="Elf"?0xc99778:race==="Dwarf"?0x9a684c:race==="Kobold"?0x76512d:0xad7658;
  ...
  for(const x of [-.105,.105]){const eye=mesh(new THREE.SphereGeometry(.027,10,7),standard(race==="Kobold"?0xb62f1f:0x19394e,0,.25));eye.position.set(x,2.59,-.285);root.add(eye);}
  if(race==="Elf")for(const x of [-1,1]){const ear=mesh(new THREE.ConeGeometry(.1,.34,10),skin);ear.position.set(x*.34,2.57,0);ear.rotation.z=x*-Math.PI/2;root.add(ear);}
  if(race==="Kobold"){const muzzle=mesh(new THREE.CapsuleGeometry(.14,.22,6,10),skin);muzzle.rotation.x=Math.PI/2;muzzle.position.set(0,2.48,-.3);root.add(muzzle);for(const x of [-1,1]){const ear=mesh(new THREE.ConeGeometry(.17,.44,10),skin);ear.position.set(x*.24,2.88,.02);ear.rotation.z=x*.28;root.add(ear);}}
