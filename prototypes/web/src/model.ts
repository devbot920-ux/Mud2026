...
export function validateWorld(value: unknown): World {
  ...
  for (const spawn of world.spawns) for (const entry of spawn.entries) if (!(entry.entity_type === "npc" ? npcs : items).has(entry.entity_id)) errors.push(`spawn ${spawn.id} references missing ${entry.entity_type} ${entry.entity_id}`);
