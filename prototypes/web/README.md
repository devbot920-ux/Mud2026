...
Open `http://127.0.0.1:4173`. The prepare step verifies SHA-256 `146D19341A7172EFA604E7E32D9277A9E86750ADE26D5637CFDA1EDBBDDE2DA4` before copying the private world export, then reads `RCI_SPEL.db` immutable/read-only and prepares a source-decoded private spell fixture. `public/private/`, `dist/`, and `node_modules/` are ignored. Never commit a prepared fixture or a production build containing one.
...
- Nearby entities and exits also produce clickable proximity actions, including quick attacks and damaging spells.
...
- Canonical hostile spawns in the cellar and excavated tunnels automatically notice and pursue players after a short entry grace period. This follows the recovered behavior-1 field shared by the giant slug, kobold, kobold thug, and kobold guard.
- On victory the mob and its body disappear immediately. Experience is banked, while deterministic prototype equipment falls onto the floor and remains in that room until collected with `E`, a proximity button, `GET <item>`, or `TAKE <item>`. World mobs may return after roughly two minutes, but only after the room is loaded again.
...
Room 3976 is a functional tutorial-room vertical slice with collision boundaries, gaze/proximity interaction, canonical descriptions, tutorial progress, text-command equivalents, and westward travel. The exported rooms through 3984 add inventory, practice-dummy combat, and working vertical traversal. The central hub adds modeled shops and civic spaces, solid major furnishings, third-person character rendering, reciprocal combat, health/movement/mana, source-aligned race and walk-of-life choices, attributes, proficiencies, experience/levels, weighted equipment, spellcasting, pursuit, victories, and hospice respawning. Confirmed original spell math now drives the casting slice; undecoded weapon fields and some derived statistics remain explicitly prototype-defined. Door open/lock state, persistence, a full economy, quests, and full-world geometry are not yet implemented.
...
Tests use only synthetic/public rule inputs. They cover contract/reference checks, navigation, targeting, commands, combat, decoded NPC aggression modes, encounter cooldown/origin keys, recovered spell formulas, pursuit and exit blocking, all eight attributes, source-aligned race/walk differences, explicit promotion and point grants, trainer costs, rest recovery, all equipment locations, progression, inventory weight, equipment-slot replacement, and camera controls.
