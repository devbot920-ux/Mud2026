...
describe("modeled geometry movement",()=>{
  ...
  it("leaves a public passage through the bank partition",()=>expect(constrainModeledMovement(4171,{x:0,z:-6.2},{x:0,z:-4.8})).toEqual({x:0,z:-4.8}));
  it("still blocks walking through the bank teller counter",()=>expect(constrainModeledMovement(4171,{x:-3,z:-6.2},{x:-3,z:-4.8})).toEqual({x:-3,z:-6.2}));
});
